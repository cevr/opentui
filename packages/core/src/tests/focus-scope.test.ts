...
import { FocusScopeRenderable } from "../renderables/FocusScope"
...
test("scope intercepts all keys - global handler does not fire", () => {
  ...
  const scope = new FocusScopeRenderable(testRenderer, { id: "scope", width: 40, height: 10 })
...
test("listener fires for every keystroke", () => {
  ...
  const scope = new FocusScopeRenderable(testRenderer, {
    id: "scope",
    width: 40,
    height: 10,
  })
...
test("re-activation - new scope works after old destroyed", () => {
  ...
  const scope1 = new FocusScopeRenderable(testRenderer, {
    id: "scope1",
    width: 40,
    height: 10,
  })
  ...
  const scope2 = new FocusScopeRenderable(testRenderer, {
    id: "scope2",
    width: 40,
    height: 10,
  })
...
test("inner scope has priority - only innermost listener fires", () => {
  ...
  const outer = new FocusScopeRenderable(testRenderer, {
    id: "outer",
    width: 60,
    height: 20,
  })
  ...
  const inner = new FocusScopeRenderable(testRenderer, {
    id: "inner",
    width: 30,
    height: 10,
  })
...
test("three levels deep - deepest fires, then middle, then outer", () => {
  ...
  const middle = new FocusScopeRenderable(testRenderer, {
    id: "middle",
    width: 50,
    height: 15,
  })
  ...
  const deep = new FocusScopeRenderable(testRenderer, {
    id: "deep",
    width: 30,
    height: 10,
  })
...
test("tab cycles forward through focusable children", async () => {
  const scope = new FocusScopeRenderable(testRenderer, { id: "scope", width: 60, height: 20 })
...
test("nested scope restore - inner restores to outer's focus", async () => {
  const outer = new FocusScopeRenderable(testRenderer, { id: "outer", width: 60, height: 20 })
  ...
  const inner = new FocusScopeRenderable(testRenderer, { id: "inner", width: 30, height: 10 })
